...
clipboard / primary selection owner も `waylandd` registry の一部として扱います。owner id だけではなく、最後に publish された `payload_id` / `source_serial` も `waylandd` 側の真実に含めます。ただし owner が既に死んでいる場合だけは、そのまま dangling owner を残さず、`compd` が復元後 focus を使って handoff を提案し、`waylandd` が apply します。この時、dead owner に紐づいていた payload metadata は引き継がず clear します。owner が `None` の場合や、まだ生きている owner を `compd` が勝手に上書きすることはしません。

...
- `scripts/run-xwin-screenshot-isolated-manifest.sh` は dev-harness / fake / isolated-displayd / config flow / negative checks を repo-local `target/xsm/` 配下の run root で再現する補助 runner であり、production displayd ではない。

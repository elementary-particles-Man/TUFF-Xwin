...
pub const MAX_WIRE_STRING_BYTES: usize = 64 * 1024;
pub const MAX_WIRE_ARRAY_BYTES: usize = 256 * 1024;
...
pub fn decode_string(bytes: &[u8], offset: &mut usize) -> Result<String> {
    ...
    *offset += 4;
    ...
    if len > MAX_WIRE_STRING_BYTES + 1 {
        return Err(WireError::ProtocolError(format!(
            "wire string exceeds {} bytes",
    ...
    // Wayland strings are null-terminated.
...
pub fn decode_array(bytes: &[u8], offset: &mut usize) -> Result<Vec<u8>> {
    ...
    *offset += 4;
    ...
    if len > MAX_WIRE_ARRAY_BYTES {
        return Err(WireError::ProtocolError(format!(
            "wire array exceeds {} bytes",
            MAX_WIRE_ARRAY_BYTES
        )));
    }
...
mod tests {
    ...
    #[test]
    fn rejects_oversized_string_and_array() {
        let mut string_offset = 0usize;
        let string_bytes = {
            let mut out = Vec::new();
            let too_long = vec![b'a'; MAX_WIRE_STRING_BYTES + 1];
            out.extend_from_slice(&((too_long.len() + 1) as u32).to_le_bytes());
            out.extend_from_slice(&too_long);
            out.push(0);
            while out.len() % 4 != 0 {
                out.push(0);
            }
        ...
        assert!(decode_string(&string_bytes, &mut string_offset).is_err());

        let mut array_offset = 0usize;
        let array_bytes = {
            let mut out = Vec::new();
            let too_long = vec![0u8; MAX_WIRE_ARRAY_BYTES + 1];
            out.extend_from_slice(&(too_long.len() as u32).to_le_bytes());
            out.extend_from_slice(&too_long);
            while out.len() % 4 != 0 {
                out.push(0);
            }
        ...
        assert!(decode_array(&array_bytes, &mut array_offset).is_err());
    }
}

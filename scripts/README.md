...
- `run-scene-recovery-demo.sh`: `displayd` の last-scene snapshot と `waylandd` の surface registry を使って、`compd` が broker rebuild できることを確認する
- `run-compd-broker-recovery.sh`: `watchdog -> sessiond` recovery execution で `compd` を restart し、startup 時に `displayd + waylandd` snapshot から scene を rebuild して再commit できることを確認する

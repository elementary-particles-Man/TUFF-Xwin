...
- `run-compd-broker-recovery.sh`: `watchdog -> sessiond` recovery execution で `compd` を restart し、startup 時に `displayd + waylandd` snapshot から scene rebuild、selection handoff、再commit までできることを確認する

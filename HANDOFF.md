...
### R2: クライアントトポロジ競合の解消 (完了)
- **非同期初期化の解消**: クライアント登録時の `query_output_topology()` によるブロックを廃止し、`TOPOLOGY_REGISTRY` 内の `Condvar` を用いて、初期トポロジのアトミックな読み取りとブロードキャストリストへの登録を同期待ちするよう修正。
- **書き込み失敗と遅延クライアントの隔離**: バッファ付きチャネルを利用し、オーバーフローや書き込みエラーが発生したクライアントのみを安全に切断・削除する仕組みを導入。共有スーパーバイザーループや正常なクライアントには影響を与えない。
- **`wl_output` のコヒーレントな状態生成**: `wayland_wire::core::HeadlessWireCore` にて、`wl_output` の初期化バッチが世代の混ざらない一貫した状態として発行されるよう修正。
- **再登録時の Global ID 衝突の解消**: 削除された ID を再利用しないよう `next_global_name` 単調増加カウンタを導入。
- **検証**: `r2_slow_client_queue_isolation` と `r2_client_write_failure_isolation` を始めとするテストを追加し、並行処理や排他制御に関する全ての決定論的な回帰テストがクリーンにパスすることを確認済み。
